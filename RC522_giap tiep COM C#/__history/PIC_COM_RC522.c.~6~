...
void quet_the()
{
   IF (MFRC522_isCard (&TagType)) //Check any card  có the moi khong 
   {                                           
   ...
   MFRC522_Halt ();  // dua module vào chê do ngu den khi the cu duoc lay ra, chong doi the
}
...
int1 doc_data_uart()
{
   ...
   if(kbhit())
   {
      ...
      if(c =='0')// the sau thong báo loi ERROR
      {
         coi_keu(0);
         tt = 3;
      }
      if(c =='L') // ngat kêt nôi phan mem
      {
         tt = 4;
      }
   }
...
void main()
{
   ...
   while(TRUE)
   {
      if(tam_KN == 0)
      {
         if(kbhit())// khi có tín hieu tu uart gui ve
         {
